...
const Concept = () => {
  ...
  const getDescriptionData = async () => {
    const resData1 = await axios.get('http://localhost:3000/description')
    setDescriptionData(resData1.data[0])
    console.log(resData1)
  }

  const getConceptData = async () => {
    const resData2 = await axios.get('http://localhost:3000/concept')
    setConceptData(resData2.data[0])
    console.log(resData2)
  }

  const getValueData = async () => {
    const resData3 = await axios.get('http://localhost:3000/value')
